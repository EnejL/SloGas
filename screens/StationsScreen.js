...
import { isStationOpen, calculateDistance } from "../utils/stationUtils";
...
const StationsScreen = ({ navigation }) => {
  ...
  const loadStations = useCallback(async () => {
    ...
    try {
      ...
      // updateWidgetData(data); // Removed, now handled by useEffect with favorites
  ...
  useEffect(() => {
    if (stations.length > 0) {
      console.log('Updating widget with favorites:', favoriteStations.length);
      updateWidgetData(favoriteStations);
    }
